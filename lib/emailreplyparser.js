...
Email.prototype = {
  ...
  // Splits the given text into a list of Fragments.  This is roughly done by
	...
	read: function(text) {
		...
		var englishPatt = /^(On\s(\n|.)*wrote:)$/mi;
		var englishDoubleOnPatt = /^(On\s(\n|.)*(^(> )?On\s)((\n|.)*)wrote:)$/mi;

		if(englishPatt.test(text) && !englishDoubleOnPatt.test(text)) {
			var reply_header = (englishPatt.exec(text))[0];
			text = text.replace(reply_header, reply_header.replace(/\n/g, ' '));
		}

		var frenchPatt = /^(Le\s(\n|.)*a écrit:)$/mi;
		var frenchDoubleOnPatt = /^(Le\s(\n|.)*(\s)((\n|.)*)a écrit :)$/mi;

		if(frenchPatt.test(text) && !frenchDoubleOnPatt.test(text)) {
			var reply_header = (frenchPatt.exec(text))[0];
			text = text.replace(reply_header, '');
		} else if (frenchDoubleOnPatt.test(text)) {
			var reply_header = (frenchDoubleOnPatt.exec(text))[0];
			text = text.replace(reply_header, '');
		}
		...
		// DATE NAME <EMAIL>:
		var GmailPatt = /^(\n.*?\s.*?\<[\n]?.*?\>\:)$/m;
		if(GmailPatt.test(text)) {
			var reply_header = (GmailPatt.exec(text))[0];
			text = text.replace(reply_header, ' ');
		}
